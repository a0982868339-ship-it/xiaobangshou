    ...
    <van-form @submit="onSubmit" style="margin-top: 12px;">
      ...
      <van-cell-group inset title="详细描述" style="margin-top: 12px;">
        <van-field
          ...
          type="textarea"
          maxlength="500"
          placeholder="请详细描述您遇到的问题或需求，如果是订单相关，请提供订单号..."
          show-word-limit
...
const typeColumns = [
  { text: '业务咨询', value: 'order' },
  { text: '支付问题', value: 'finance' },
  { text: '投诉建议', value: 'complaint' },
  { text: '账号安全', value: 'other' },
  { text: '其他问题', value: 'other' }
]

const form = ref({
  title: '',
  category: 'order', // 对应后端分类
  typeName: '业务咨询', // 显示用的名称
  content: '',
  contact: ''
})
...
const onSubmit = async () => {
  ...
  try {
    ...
    showSuccessToast('工单提交成功，我们将尽快处理')
    router.back()
  } catch (e) {
    console.error(e)
  } finally {
